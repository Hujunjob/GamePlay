...
object CameraConfigurationUtils {
    ...
    fun setBestExposure(
        ...
        if ((minExposure != 0 || maxExposure != 0) && step > 0.0f) { // Set low when light is on
            ...
            // Clamp value:
            compensationSteps =
                Math.max(Math.min(compensationSteps, maxExposure), minExposure)
    ...
    fun setBestPreviewFPS(
        ...
        val supportedPreviewFpsRanges =
            parameters.supportedPreviewFpsRange
        Log.i(
            TAG,
            "Supported FPS ranges: " + toString(supportedPreviewFpsRanges)
        )
        if (supportedPreviewFpsRanges != null && !supportedPreviewFpsRanges.isEmpty()) {
            ...
            if (suitableFPSRange == null) {
                ...
                if (Arrays.equals(currentFpsRange, suitableFPSRange)) {
                    Log.i(
                        TAG,
                        "FPS range already set to " + Arrays.toString(suitableFPSRange)
                    )
                } else {
                    Log.i(
                        TAG,
                        "Setting FPS range to " + Arrays.toString(suitableFPSRange)
                    )
    ...
    fun setFocusArea(parameters: Camera.Parameters) {
        if (parameters.maxNumFocusAreas > 0) {
            Log.i(
                TAG,
                "Old focus areas: " + toString(parameters.focusAreas)
            )
            val middleArea =
                buildMiddleArea(AREA_PER_1000)
            Log.i(
                TAG,
                "Setting focus area to : " + toString(middleArea)
            )
    ...
    fun setMetering(parameters: Camera.Parameters) {
        if (parameters.maxNumMeteringAreas > 0) {
            Log.i(
                TAG,
                "Old metering areas: " + parameters.meteringAreas
            )
            val middleArea =
                buildMiddleArea(AREA_PER_1000)
            Log.i(
                TAG,
                "Setting metering area to : " + toString(middleArea)
            )
    ...
    private fun indexOfClosestZoom(
        ...
        Log.i(
            TAG,
            "Chose zoom ratio of " + ratios[closestIndex] / 100.0
        )
        return closestIndex
    }
    ...
    private fun findSettableValue(
        ...
        Log.i(
            TAG,
            "Requesting " + name + " value from among: " + Arrays.toString(desiredValues)
        )
    ...
    fun collectStats(flattenedParams: CharSequence?): String {
        ...
        if (flattenedParams != null) {
            val params =
                SEMICOLON.split(flattenedParams)
            Arrays.sort(params)
            for (param in params) {
